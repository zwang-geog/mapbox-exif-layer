# Add weather raster layer — MapLibre GL JS
...
`SmoothRaster` works with MapLibre **globe** projection:
...
Initialize a single layer with the default variable. Set **`mapRuntime: 'maplibre'`** on every `SmoothRaster` instance.
...
Prefer `style.load` when the basemap style can change or MapLibre globe projection is used; otherwise prefer `load`.
